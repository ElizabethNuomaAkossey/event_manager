...
def show_signup_page():
    with ui.element("div").classes("w-screen h-screen flex m-0 p-0 border rounded-2xl overflow-hidden"):
        ...
        with ui.element("div").classes("relative h-screen w-[55%] bg-gray-100 m-0 p-0 flex items-center justify-center"):
            with ui.card().classes("w-[80%] h-[100%]  bg-transparent flex flex-col items-center m-0 pt-8 shadow-none border-none"):
                
                # Top row: Event + Hive
                with ui.element("div").classes("flex justify-center items-center space-x-2 mt-1"):
                    ui.label("Event").classes("text-sm font-bold text-black")
                    ui.label("Hive").classes("text-sm font-bold text-purple-500")

                # Subtitle
                ui.label("Sign Up to Event Hive").classes("text-xl font-bold text-center m-0 p-0")

                # Form section
                with ui.element("div").classes("w-full flex flex-col space-y-2 px-1"):
                    
                    # Name
                    with ui.element("div").classes("flex flex-col w-full"):
                        ui.label("YOUR NAME").classes("text-xs mb-1 text-left")
                        ui.input(
                    ...
                    with ui.element("div").classes("flex flex-col w-full"):
                        ui.label("PASSWORD").classes("text-xs mb-1 text-left")
                        ui.input(
                    ...
                    # Confirm Password
                    with ui.element("div").classes("flex flex-col w-full"):
                        ui.label("CONFIRM PASSWORD").classes("text-xs mb-1 text-left")
                        ui.input(
                            placeholder="Enter your password", password_toggle_button ="yes"
